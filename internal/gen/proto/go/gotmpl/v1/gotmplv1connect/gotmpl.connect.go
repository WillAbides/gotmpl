// Code generated by protoc-gen-connect-go. DO NOT EDIT.
//
// Source: gotmpl/v1/gotmpl.proto

package gotmplv1connect

import (
	context "context"
	errors "errors"
	connect_go "github.com/bufbuild/connect-go"
	v1 "github.com/willabides/gotmpl/internal/gen/proto/go/gotmpl/v1"
	http "net/http"
	strings "strings"
)

// This is a compile-time assertion to ensure that this generated file and the connect package are
// compatible. If you get a compiler error that this constant is not defined, this code was
// generated with a version of connect newer than the one compiled into your binary. You can fix the
// problem by either regenerating this code with an older version of connect or updating the connect
// version compiled into your binary.
const _ = connect_go.IsAtLeastVersion0_1_0

const (
	// GotmplServiceName is the fully-qualified name of the GotmplService service.
	GotmplServiceName = "gotmpl.v1.GotmplService"
)

// GotmplServiceClient is a client for the gotmpl.v1.GotmplService service.
type GotmplServiceClient interface {
	// Execute executes a go template.
	Execute(context.Context, *connect_go.Request[v1.ExecuteRequest]) (*connect_go.Response[v1.ExecuteResponse], error)
}

// NewGotmplServiceClient constructs a client for the gotmpl.v1.GotmplService service. By default,
// it uses the Connect protocol with the binary Protobuf Codec, asks for gzipped responses, and
// sends uncompressed requests. To use the gRPC or gRPC-Web protocols, supply the connect.WithGRPC()
// or connect.WithGRPCWeb() options.
//
// The URL supplied here should be the base URL for the Connect or gRPC server (for example,
// http://api.acme.com or https://acme.com/grpc).
func NewGotmplServiceClient(httpClient connect_go.HTTPClient, baseURL string, opts ...connect_go.ClientOption) GotmplServiceClient {
	baseURL = strings.TrimRight(baseURL, "/")
	return &gotmplServiceClient{
		execute: connect_go.NewClient[v1.ExecuteRequest, v1.ExecuteResponse](
			httpClient,
			baseURL+"/gotmpl.v1.GotmplService/Execute",
			opts...,
		),
	}
}

// gotmplServiceClient implements GotmplServiceClient.
type gotmplServiceClient struct {
	execute *connect_go.Client[v1.ExecuteRequest, v1.ExecuteResponse]
}

// Execute calls gotmpl.v1.GotmplService.Execute.
func (c *gotmplServiceClient) Execute(ctx context.Context, req *connect_go.Request[v1.ExecuteRequest]) (*connect_go.Response[v1.ExecuteResponse], error) {
	return c.execute.CallUnary(ctx, req)
}

// GotmplServiceHandler is an implementation of the gotmpl.v1.GotmplService service.
type GotmplServiceHandler interface {
	// Execute executes a go template.
	Execute(context.Context, *connect_go.Request[v1.ExecuteRequest]) (*connect_go.Response[v1.ExecuteResponse], error)
}

// NewGotmplServiceHandler builds an HTTP handler from the service implementation. It returns the
// path on which to mount the handler and the handler itself.
//
// By default, handlers support the Connect, gRPC, and gRPC-Web protocols with the binary Protobuf
// and JSON codecs. They also support gzip compression.
func NewGotmplServiceHandler(svc GotmplServiceHandler, opts ...connect_go.HandlerOption) (string, http.Handler) {
	mux := http.NewServeMux()
	mux.Handle("/gotmpl.v1.GotmplService/Execute", connect_go.NewUnaryHandler(
		"/gotmpl.v1.GotmplService/Execute",
		svc.Execute,
		opts...,
	))
	return "/gotmpl.v1.GotmplService/", mux
}

// UnimplementedGotmplServiceHandler returns CodeUnimplemented from all methods.
type UnimplementedGotmplServiceHandler struct{}

func (UnimplementedGotmplServiceHandler) Execute(context.Context, *connect_go.Request[v1.ExecuteRequest]) (*connect_go.Response[v1.ExecuteResponse], error) {
	return nil, connect_go.NewError(connect_go.CodeUnimplemented, errors.New("gotmpl.v1.GotmplService.Execute is not implemented"))
}
